 ...
 * 用法：node scripts/check-auto-translate-modules.mjs
 */
...
const tsfPath = resolve(__dirname, "../../../TypeScriptFrontend/app/server/translateV4/moduleCatalog.ts");
let tsfModules = [];
try {
  ...
  console.warn("跳过 TSF 校验（路径不可读）:", e.message);
}
...
function diff(label, actual) {
  ...
  console.log(`✅ ${label} 与 v4 自动翻译模块一致 (${actual.length} 个，不含 EMAIL_TEMPLATE)`);
  return true;
}
...
if (canonical.includes("EMAIL_TEMPLATE")) {
  console.error("❌ v4-auto-translate-modules.json 不应包含 EMAIL_TEMPLATE");
  ok = false;
}
