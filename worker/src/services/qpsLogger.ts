 ...
 *
 * Each flush writes one document to translation_v4_qps_logs (partition: jobId).
 * Flush triggers:
...
async function writeSnapshot(doc: QpsSnapshot): Promise<void> {
...
export class QpsLogger {
  ...
  /** Take a snapshot and write to Cosmos. Best-effort — errors are logged but not thrown. */
  async flush(): Promise<void> {
    ...
    // Skip flush if nothing happened at all
    if (shopify.calls === 0 && (llm === null || llm.calls === 0)) return;

    const doc: QpsSnapshot = {
      id: `${this.jobId}:${now}`,
      jobId: this.jobId,
  ...
  stop(): void {
    ...
    // Fire-and-forget final snapshot
    this._finalFlush().catch(() => {});
  }

  private async _finalFlush(): Promise<void> {
    this.stopped = false; // temporarily re-enable to allow flush
    await this.flush();
    this.stopped = true;
  }
  ...
  private _takeLlmDelta(durationSec: number): QpsSnapshot["llm"] {
    ...
    // weighted avg latency across slots
