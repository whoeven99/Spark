...
export default function Subscriptions() {
  ...
  const load = useCallback(
    (params: { search?: string; status?: string; plan?: string; interval?: string }) => {
      setLoading(true);
  ...
  const applyFilters = useCallback(
    (overrides: { search?: string; status?: string; plan?: string; interval?: string }) => {
      load({
        ...
        interval: overrides.interval !== undefined ? overrides.interval : intervalFilter,
      });
    },
  ...
  const activeSubs = stats?.byStatus["ACTIVE"] ?? 0;
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "status",
      render: (v: string) => (
        <Badge status={STATUS_BADGE[v] ?? "default"} text={<Tag color={STATUS_COLOR[v] ?? "default"}>{v}</Tag>} />
      ),
    },
    {
      title: "计费周期",
    ...
    },
    {
      ...
      render: (v: string | null) => {
        ...
        const color = days <= 0 ? "#ff4d4f" : days <= 7 ? "#ff7a45" : days <= 30 ? "#fa8c16" : undefined;
        return (
    ...
    },
    {
      ...
      key: "usagePercent",
      sorter: (a: SubscriptionRow, b: SubscriptionRow) => {
        const totalA = a.subscriptionTokens + a.purchasedTokens + a.trialTokens;
        const totalB = b.subscriptionTokens + b.purchasedTokens + b.trialTokens;
        const pctA = totalA > 0 ? a.usedTokens / totalA : 0;
        const pctB = totalB > 0 ? b.usedTokens / totalB : 0;
        return pctA - pctB;
      },
    ...
    <div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        {[
          ...
          { title: "活跃订阅", value: activeSubs, color: "#52c41a" },
          { title: "即将到期（30天内）", value: stats?.expiringSoon ?? 0, color: "#fa8c16" },
              ...
              columns={[
                {
                  ...
                  dataIndex: "planKey",
                  render: (v: string | null) =>
                    v ? <Tag color="purple">{v}</Tag> : <span style={{ color: "#bbb" }}>无套餐</span>,
                },
                {
                  title: "活跃",
                ...
                {
                  title: "活跃率",
                  render: (_: unknown, r: { planKey: string | null; activeCount: number; total: number }) => (
                    <Progress
                      percent={r.total > 0 ? Math.round((r.activeCount / r.total) * 100) : 0}
                      size="small"
                      style={{ minWidth: 80 }}
                    />
                  ),
      ...
      >
        ...
        <Space wrap style={{ marginTop: 12, marginBottom: 16 }}>
          <Input
            ...
            onClear={() => {
              setSearch("");
              load({ status: statusFilter, plan: planFilter, interval: intervalFilter });
            }}
            style={{ width: 240 }}
          />
          <Select
            ...
            options={["ACTIVE", "PENDING", "CANCELLED", "EXPIRED", "FROZEN"].map((s) => ({
              value: s,
              label: s,
            }))}
          />
      ...
      <Drawer
        title={
          <span>
            ...
            {selected?.status && (
              <Tag color={STATUS_COLOR[selected.status] ?? "default"} style={{ marginLeft: 4 }}>
                {selected.status}
              </Tag>
            )}
          </span>
            ...
            items={billingLogs.map((log, i) => ({
              ...
              children: (
                <div>
                  ...
                  {log.planKey && (
                    <Tag style={{ marginLeft: 8 }}>{log.planKey}</Tag>
                  )}
                  <br />
                  <Typography.Text
                    style={{
