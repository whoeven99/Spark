...
本文档总结 Spark 当前主应用、Worker 与内部 Admin 已覆盖的数据层和功能层，便于后续做信息架构、首页与页面归并时统一口径。
...
| 商品与商品变体 | Shopify Admin GraphQL | 商品搜索、商品详情、商品文案生成、商品质量评分、翻译对象选择 |
| 店铺语言与 locale | Shopify Admin GraphQL | 商品文案目标语言、翻译任务源语言与目标语言 |
| Shopify 内容对象 | Shopify Admin GraphQL | 翻译 V4 模块对象、文章/博客/页面等内容读取 |
...
| 翻译 V4 任务 | Cosmos / Redis / Blob / Worker | 翻译任务元数据、阶段进度、结果文件、写回状态 |
| 翻译术语与店铺画像 | Cosmos / Blob / Shopify | 术语表、品牌语气、本地化约束 |
...
| 每日待办 | 每日经营巡检、诊断证据、四象限任务、任务状态更新、昨日复盘 | `/app/today/diagnosis` |
...
| 多语言翻译 | 创建翻译 V4 任务、选择模块/语言、术语表、进度查看、审核/写回 | `/app/studio/translate` |
| 统一任务中心 | 合并 AI 任务与翻译 V4 任务，查看当前/历史、日志、结果、失败重试入口 | `/app/tasks` |
...
| 广告渠道配置 | Meta、Google Ads、TikTok Ads、Microsoft Ads 凭证配置 | `/app/settings/channels` |
| 广告洞察 | Meta / Google / TikTok 广告系列→广告组→广告实时指标（7/14/30 天） | `/app/settings/ads-insights` |
...
| Worker 翻译流水线 | init、translate、writeback、email、自动翻译扫描、空任务清理、部署接管 | `worker/src/*` |
| 内部 Admin | 店铺、翻译任务、用量、订阅、收入、Agent 执行、客服、日志、巡检、定价 | `/admin` |

## 当前信息架构归纳

| 一级目的地 | 应放能力 | 用户心智 |
| --- | --- | --- |
| 首页 | 今日结果、重点风险、推荐动作、最近任务、AI 输入 | 今天店铺怎么样，我下一步做什么 |
| 经营 | 概览、每日待办、订单风险、退款、库存、履约、ROI、客户价值 | 看结果、找问题、排优先级 |
| 创作 | 商品文案、图片生成、图片翻译、多语言翻译、术语表、质量评分 | 生产和优化内容资产 |
| 任务 | 所有异步任务、日志、结果、审核、写回、失败重试 | 所有后台工作跑到哪了 |
| 设置 | 计费、数据同步、广告、物流、反馈 | 低频配置和基础设施 |
| 内部 Admin | 运营监控、客服、收入、日志、定价、巡检 | 内部团队管理 Spark |

## 首页目标

首页不应只是聊天页，也不应是复杂报表页。首页的目标是让用户在 10 秒内回答三个问题：
...
2. 经营摘要：销售额、订单数、退款率、库存风险、待处理任务等 4-6 个关键指标。
