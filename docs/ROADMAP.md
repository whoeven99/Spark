...
- [x] GDPR `customers/redact` / `shop/redact`：卸载与 redact 走归档+Turso 删除（`archiveAndPurgeShopData`）；`PromoClaimLedger` / `ReferralClaim` / `ReferralInstall` / `DevStoreSubscribeAllowlist` 保留
