...
- TikTok Pixel（Ads Catalog）：
  ...
  - 测试事件：保存 / Go to Online Store → 凭证 + metafield `testEventCode` + `storefrontTrackUrl`；店面浏览/加购经公开端点双发 Events API（带 test_event_code）；删除清空，恢复正式事件
  - 服务端：`orders/paid` 按勾选上报 `CompletePayment`（Events API `pixel/track`；凭证有 Test Event Code 时带 `test_event_code`）
  - 店面双发：`POST /api/ads-catalog/tiktok-storefront-track`（仅测试模式）
