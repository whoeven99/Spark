...
本文档用于将“每日诊断报告 + 每日待办任务”统一收敛为一套可落地的产品设计与数据设计文档。

本文档重点回答 4 个问题：

- 页面应该如何按 `紧急 / 重要` 四象限组织工作
- 每一个业务点如何抽象为一个 `1 / 2 / 3` 的标准工作流
- 每一个诊断点如何定义输入数据、计算口径、推理逻辑和输出任务
- 第一阶段应优先实现哪些数据与规则，避免功能过早发散
...
该设计的核心不是再做一份“日报”，而是把每天的经营问题和运营动作统一组织为一套可执行的工作系统。
