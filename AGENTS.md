...
- **Theme App Extension（`extensions/spark-tiktok-pixel/`）**：店面 App embed 注入 TikTok `ttq`；Pixel / Events API 配置在 Ads Catalog，经 Shop metafield `spark_tiktok.pixel_config` 下发。测试事件 Code 写入同一 metafield（及可选 URL/`sessionStorage`）；测完须删除以免污染正式数据。
重要边界：
