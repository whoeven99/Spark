...
- Spark tab「小红书图文」：`/xhs-promo` → `admin/src/pages/XhsPromo.tsx` + `admin/server/routes/xhsPromo.ts`。所有登录用户可用。分步确认：选题 → 可选标题（可改）→ 文案（可改）→ 封面与滑页分开出图。接口拆开：`/titles`、`/copy`、`/cover`、`/cards`；改文案或调某一侧提示词不会重跑另一侧。默认文案 DeepSeek（`DEEPSEEK_API_KEY`），默认封面火山方舟 Seedream（`VOLC_ARK_API_KEY` + `VOLC_ARK_IMAGE_MODEL`，默认 `doubao-seedream-5-0-pro-260628`）。豆包写文仅在显式配置 `VOLC_ARK_TEXT_MODEL` 时出现在选项里，不要写死 `doubao-seed-1-6-251015`。封面还可选手 GPT 或模板 SVG。标题/文案/封面/滑页提示词各自收起可改；未改过则按当前步骤重算。提示词按黑/白/荧光黄绿信息卡。滑页卡片走独立提示词 + SVG 模板排字，不走 Seedream，不接 Playwright。不接小红书发帖。不要用旧视觉 `HUOSHAN_*` / `CVProcess`。
