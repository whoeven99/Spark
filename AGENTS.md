...
- Spark tab「小红书图文」：`/xhs-promo` → `admin/src/pages/XhsPromo.tsx` + `admin/server/routes/xhsPromo.ts`。所有登录用户可用。一次生成三件：正文 200–400 字、瑞士信息卡封面、2–4 张滑页卡片。文案仅在显式配置 `VOLC_ARK_TEXT_MODEL`（方舟对话 Model ID 或 `ep-`）时用豆包，与封面共用 `VOLC_ARK_API_KEY`；未配置则 `DEEPSEEK_API_KEY`，再否则 `OPENAI_API_KEY`。不要写死 `doubao-seed-1-6-251015`。页面可在已配置的文案模型间切换。封面优先 Seedream（`VOLC_ARK_IMAGE_MODEL`，默认 `doubao-seedream-5-0-pro-260628`），提示词按黑/白/荧光黄绿信息卡，否则 GPT，再失败回退模板 SVG。滑页卡片由 `cards` 槽 + SVG 模板渲染，不走 Seedream，不接 Playwright。不接小红书发帖。不要用旧视觉 `HUOSHAN_*` / `CVProcess`。
