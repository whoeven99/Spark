...
- Spark tab「生成小红书图文」：`/xhs-promo` → `admin/src/pages/XhsPromo.tsx` + `admin/server/routes/xhsPromo.ts`。所有登录用户可用。文案优先 `DEEPSEEK_API_KEY`（否则 `OPENAI_API_KEY`），封面走主应用同一套 GPT 文生图（`OPENAI_IMAGE_API_KEY` / `OPENAI_IMAGE_BASE_URL` / `OPENAI_IMAGE_MODEL`，默认 `gpt-image-2`），失败回退模板 SVG。页面顶部标注实际使用的文案/封面模型。不接小红书发帖。不走火山文生图。
