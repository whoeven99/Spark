...
- Spark tab「生成小红书图文」：`/xhs-promo` → `admin/src/pages/XhsPromo.tsx` + `admin/server/routes/xhsPromo.ts`。所有登录用户可用。文案优先 `DEEPSEEK_API_KEY`（否则 `OPENAI_API_KEY`），封面走火山方舟 `VOLC_ARK_API_KEY` 或视觉 `HUOSHAN_*` / `VOLC_*`，失败回退模板 SVG。页面顶部标注实际使用的文案/封面模型。不接小红书发帖。
