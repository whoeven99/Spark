...
- Spark tab「生成小红书图文」：`/xhs-promo` → `admin/src/pages/XhsPromo.tsx` + `admin/server/routes/xhsPromo.ts`。所有登录用户可用。文案仅在显式配置 `VOLC_ARK_TEXT_MODEL`（方舟对话 Model ID 或 `ep-`）时用豆包，与封面共用 `VOLC_ARK_API_KEY`；未配置则 `DEEPSEEK_API_KEY`，再否则 `OPENAI_API_KEY`。不要写死 `doubao-seed-1-6-251015`（该 ID 对当前账号不可用）。页面可在已配置的文案模型间切换。封面优先火山方舟 Seedream（`VOLC_ARK_IMAGE_MODEL`，默认 `doubao-seedream-5-0-pro-260628`），否则 GPT 文生图，再失败回退模板 SVG。页面顶部标注实际使用的文案/封面模型。不接小红书发帖。不要用旧视觉 `HUOSHAN_*` / `CVProcess`。
