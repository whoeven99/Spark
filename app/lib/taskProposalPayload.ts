...
// ─── 商品 Metafield 批量改写（受控写回：dry-run 预览 + 二次确认才写 Shopify） ─

export const BULK_METAFIELD_EDIT_SKILL_ID = "bulk_metafield_edit";

/**
 * `unset` 与批量上下架、批量改合集同理：设值与清空方向相反，
 * 默认帮用户选一个的代价是把商户填好的字段整批抹掉。
 * 它不是合法规则值，`parseBulkMetafieldEditRule` 会拒绝并提示先选动作。
 */
export const BULK_METAFIELD_EDIT_ACTION_OPTIONS: Array<{ value: string; label: string }> = [
  { value: "unset", label: "请选择：设置值或清空" },
  { value: "set", label: "设置为指定值" },
  { value: "clear", label: "清空该字段" },
];

export const BULK_METAFIELD_EDIT_FILL_MODE_OPTIONS: Array<{ value: string; label: string }> = [
  { value: "false", label: "覆盖已有值" },
  { value: "true", label: "只填当前为空的" },
];

/**
 * 由商品列表 + 自定义字段构造「批量改 Metafield」提案。
 * 确认后只创建一个 dry-run 任务（读 + 算），真正写 Shopify 要在审核弹窗再确认一次。
 *
 * fieldOptions 由 Skill 在开卡时从 Shopify 预取，只含本能力支持的标量类型定义；
 * list / reference / rich text 不进下拉，因为一个输入框填不对。
 */
export function buildBulkMetafieldEditProposal(args: {
  products: Array<{ id: string; title: string; imageUrl?: string | null }>;
  action?: string;
  fieldKey?: string;
  value?: string;
  ...
  /** 店铺定义多于本次预取数量：提示用户可以直接说字段名来缩小范围 */
  fieldsTruncated?: boolean;
}): TaskProposalPayload {
  const fieldOptions = args.fieldOptions ?? [];
  const fieldKey =
    args.fieldKey && fieldOptions.some((o) => o.value === args.fieldKey)
      ? args.fieldKey
  ...
  return {
    ...
    title: "批量修改商品自定义字段",
    summary:
      "按所选自定义字段逐个商品算出目标值并生成变更清单（可导出 CSV），确认无误后才写回店铺。本步骤不会修改任何商品。",
    targets: {
      kind: "products",
      items: args.products.map((p) => ({
        id: p.id,
    ...
    params: [
      {
        ...
        placeholder: args.fieldsTruncated
          ? "输入关键词筛选；字段较多时可直接在对话里说出字段名"
          : "输入关键词筛选自定义字段",
      },
      {
        key: "metafieldAction",
        label: "操作方式",
        type: "select",
        value: pickOption(BULK_METAFIELD_EDIT_ACTION_OPTIONS, args.action, "unset"),
        options: BULK_METAFIELD_EDIT_ACTION_OPTIONS,
      },
      {
        key: "value",
        label: "写入的值",
        type: "text",
        value: args.value ?? "",
        placeholder: "可用 {title} {vendor} {productType} 占位符；选择「清空」时留空即可",
      },
      {
        key: "onlyFillEmpty",
        label: "写入范围",
        type: "select",
        value: onlyFillEmpty,
        options: BULK_METAFIELD_EDIT_FILL_MODE_OPTIONS,
      },
    ],
  };
}
...
export const BULK_INVENTORY_IMPORT_SKILL_ID = "bulk_inventory_import";

/**
 * 由「已上传的表格 + 目标地点 + 列映射」构造库存导入提案。
 *
 * 与另外两个表格导入的区别：库存量是「库存项 × 地点」的交叉数据，
 * 所以多一个必选的地点字段。地点只有一个时预选它——大多数店铺就是这种情况，
...
export function buildBulkInventoryImportProposal(args: {
  ...
  const requested =
    args.locationId && locationOptions.some((o) => o.value === args.locationId)
      ...
      : "";
  const locationId =
    requested || (locationOptions.length === 1 ? locationOptions[0].value : "");
  return {
    version: TASK_PROPOSAL_VERSION,
    proposalId: `tp-${typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : Date.now()}`,
    ...
    params: [
      ...
      },
      {
        ...
        label: "库存数量列",
        type: "text",
        value: args.quantityColumn ?? "",
        placeholder: "表格里存放库存件数的列名，如：库存、现有库存、Quantity",
      },
    ],
  };
}
