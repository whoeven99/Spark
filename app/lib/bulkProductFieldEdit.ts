...
export const SHOPIFY_SEO_TITLE_MAX_CHARS = 70;
export const SHOPIFY_SEO_DESCRIPTION_MAX_CHARS = 320;

export const BULK_PRODUCT_FIELD_EDIT_MAX_PRODUCTS = 200;

/** Shopify 商品标题硬上限（字符数）。 */
export const SHOPIFY_PRODUCT_TITLE_MAX_LENGTH = 255;
...
function seoMaxChars(field: BulkProductFieldEditField): number | null {
  if (field === "seoTitle") return SHOPIFY_SEO_TITLE_MAX_CHARS;
  if (field === "seoDescription") return SHOPIFY_SEO_DESCRIPTION_MAX_CHARS;
  return null;
}
...
export function computeProductFieldChange(
  ...
  if (rule.field === "title" && afterValue.length > SHOPIFY_PRODUCT_TITLE_MAX_LENGTH) {
    return { ...base, skipped: true, skipReason: "too_long" };
  }
  if (beforeValue === afterValue) {
    return { ...base, skipped: true, skipReason: "no_change" };
  }
  const maxChars = seoMaxChars(rule.field);
  if (maxChars != null && afterValue && afterValue.length > maxChars) {
