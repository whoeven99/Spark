...
const DATA_KEYS: Array<{ key: string; operation: ProductImportOperation }> = [
  ...
  { key: "compare_at", operation: "price" },
...
export function countUnusedImportColumns(
  preview: Pick<
    ProductImportSheetPreview,
    "ignoredColumns" | "unknownColumns" | "unsupportedColumns"
  >,
): number {
  return (
    preview.ignoredColumns.length +
    preview.unknownColumns.length +
    preview.unsupportedColumns.length
  );
}
...
export function buildProductImportSheetPreview(args: {
  ...
  return {
    ...
    issues: actionableIssues.slice(0, PRODUCT_IMPORT_SHEET_PREVIEW_ISSUE_LIMIT),
...
export function coerceProductImportSheetPreview(raw: unknown): ProductImportSheetPreview | null {
  ...
  return {
    ...
    ignoredColumns: coerceStringList(record.ignoredColumns),
    ...
    issues: filterActionableImportIssues(coerceProductImportIssues(record.issues)),
