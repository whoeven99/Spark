...
export type ProductImportPreviewKind = "change" | "skip" | "issue" | "failure";
...
export type ProductImportPreviewRow = {
  ...
  applyError?: ProductImportApplyError;
};
...
function pricePreviewRows(rows: BulkPriceEditRow[]): SplitRows {
  ...
  for (const row of rows) {
    const skipReason = row.skipReason ? { skipReason: row.skipReason } : {};
    if (row.priceChanged || row.skipped) {
      out.push({
        key: `price-${row.variantId}`,
        kind: row.priceChanged ? "change" : "skip",
    ...
    if (row.compareAtChanged) {
      out.push({
        key: `price-compareAt-${row.variantId}`,
        kind: "change",
        productTitle: row.productTitle,
        operation: "price",
        field: "compareAt",
        beforeValue: row.beforeCompareAt ?? "",
        afterValue: row.afterCompareAt ?? "",
      });
    }
  }
...
export function flattenProductImportIssues(issues: ProductImportIssue[]): ProductImportPreviewRow[] {
  return collapseRepeatedImportIssues(issues).map((issue, index) => {
    const rowLabel = issue.rowLabel || (issue.rowNumber > 0 ? String(issue.rowNumber) : "");
    return {
      key: `issue-${issue.rowNumber}-${issue.code}-${issue.column ?? ""}-${index}`,
      kind: "issue" as const,
      productTitle: issue.productTitle || (rowLabel ? `#${rowLabel}` : ""),
      operation: "issue",
      field: issue.column ?? "",
...
export function flattenProductImportApplyErrors(errors: ProductImportApplyError[]): ProductImportPreviewRow[] {
  return errors.map((error, index) => ({
    key: `apply-fail-${error.operation}-${error.variantId ?? error.productId ?? index}-${index}`,
