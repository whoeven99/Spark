...
const NATIVE_METAFIELD_HEADER =
  /^(.*)\s+\((product|variant)\.metafields\.([^.]+)\.([^)]+)\)$/i;
...
export function parseImportMetafieldHeader(header: string): ProductImportMetafieldColumn | null {
  ...
  const native = trimmed.match(NATIVE_METAFIELD_HEADER);
  if (native) {
    const owner: BulkMetafieldOwner = native[2]?.toLowerCase() === "variant" ? "variant" : "product";
    const namespace = native[3]?.trim() ?? "";
    const key = native[4]?.trim() ?? "";
...
function stripLeadingApostrophe(value: string): string {
  if (!value.startsWith("'") || value.length < 2) return value;
  return value.slice(1).trim();
}

function normalizeListSingleLine(raw: string): { ok: true; value: string } | { ok: false } {
  const source = raw.startsWith("[") ? raw : stripLeadingApostrophe(raw);
  ...
  const items = source
    .split(/[,，]/)
    .map((item) => stripLeadingApostrophe(item.trim()))
    .filter((item) => item.length > 0);
  return { ok: true, value: JSON.stringify(items) };
}

function normalizeScalarMetafieldValue(
  type: string,
  trimmed: string,
  original: string,
  ...
  if (type === "single_line_text_field") return { ok: true, value: trimmed };
  if (type === "multi_line_text_field") return { ok: true, value: original };
  return { ok: false };
}
...
export function normalizeMetafieldValue(
  ...
  if (type === "list.single_line_text_field") return normalizeListSingleLine(trimmed);
  const parsed = normalizeScalarMetafieldValue(type, trimmed, raw);
  if (parsed.ok) return parsed;
  const stripped = stripLeadingApostrophe(trimmed);
  if (stripped === trimmed) return { ok: false };
  return normalizeScalarMetafieldValue(type, stripped, raw);
}
...
export function computeMetafieldChange(args: {
  ...
  const beforeComparable =
    args.type === "list.single_line_text_field"
      ? normalizeMetafieldValue(args.type, args.beforeValue)
      : { ok: true as const, value: args.beforeValue };
  if (beforeComparable.ok && normalized.value === beforeComparable.value) {
