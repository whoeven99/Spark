/**
 * POST /api/bulk-price-edit
 *
 * 批量调价的写回入口，也是本功能唯一会向 Shopify 发 mutation 的地方。
 * 前置条件（全部满足才写）：
 *   1. 任务属于当前店铺、类型为 bulk_price_edit、状态为 pending_review；
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!task || task.taskType !== "bulk_price_edit") {
  ...
  try {
    ...
    console.error("[api.bulk-price-edit] apply failed:", e);
