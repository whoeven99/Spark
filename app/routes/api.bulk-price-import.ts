/**
 * POST /api/bulk-price-import
 *
 * 价目表导入的写回入口。真正的 mutation 复用 `applyBulkPriceEdit`
 * （全仓库唯一 `productVariantsBulkUpdate` 调用处），这里只做门禁：
 *   1. 任务属于当前店铺、类型为 bulk_price_import、状态为 pending_review；
...
import { applyBulkPriceEdit } from "../server/bulkPriceEdit/bulkPriceEditApply.server";
import { coerceBulkPriceEditRows } from "../lib/bulkPriceEdit";
import type { BulkPriceImportApplyResponse } from "../lib/aiTaskTypes";
...
function fail(error: string, status: number) {
  return data<BulkPriceImportApplyResponse>({ ok: false, error }, { status });
}
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!task || task.taskType !== "bulk_price_import") {
  ...
  // 导入行结构上就是 BulkPriceEditRow，校验与写回都能直接复用调价那一套
  const rows = coerceBulkPriceEditRows(rawResult.rows);
  const writableRows = rows.filter(
    (row) => !row.skipped && (row.priceChanged || row.compareAtChanged),
  );
  ...
  try {
    ...
    return data<BulkPriceImportApplyResponse>({
      ok: true,
      succeeded: outcome.succeeded,
      failed: outcome.failed,
    });
  } catch (e) {
    console.error("[api.bulk-price-import] apply failed:", e);
