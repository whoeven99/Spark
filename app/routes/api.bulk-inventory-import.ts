 ...
 *
 * 库存导入的写回入口。真正的 mutation 在 `applyBulkInventoryImport`
 * （全仓库唯一 `inventorySetQuantities` 调用处），这里只做门禁：
 *   1. 任务属于当前店铺、类型为 bulk_inventory_import、状态为 pending_review；
 *   2. 请求体带 confirm: true（用户在审核弹窗点过「确认写回」）；
 *   3. 该任务没有正在进行中的写回；
 *   4. 试算结果里带着目标地点——库存量是「库存项 × 地点」的交叉数据，缺地点写不了。
 */
...
} from "../server/bulkInventoryImport/bulkInventoryImportApply.server";
import { coerceBulkInventoryImportRows } from "../lib/bulkInventoryImport";
import type { BulkInventoryImportApplyResponse } from "../lib/aiTaskTypes";
...
function fail(error: string, status: number) {
  return data<BulkInventoryImportApplyResponse>({ ok: false, error }, { status });
}
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!task || task.taskType !== "bulk_inventory_import") {
  ...
  const locationId = typeof rawResult.locationId === "string" ? rawResult.locationId : "";
  if (!locationId) {
    return fail("试算结果缺少目标地点，请重新发起导入", 400);
  }
  ...
  const writableRows = selectWritableInventoryRows(
    coerceBulkInventoryImportRows(rawResult.rows),
  );
  ...
  try {
    ...
    return data<BulkInventoryImportApplyResponse>({
      ok: true,
    ...
    console.error("[api.bulk-inventory-import] apply failed:", e);
