/**
 * POST /api/bulk-status-edit
 *
 * 批量上下架的写回入口，也是本功能唯一会向 Shopify 发 mutation 的地方。
 * 前置条件（全部满足才写）：
 *   1. 任务属于当前店铺、类型为 bulk_status_edit、状态为 pending_review；
...
} from "../server/bulkStatusEdit/bulkStatusEditApply.server";
import { coerceBulkStatusEditRows } from "../lib/bulkStatusEdit";
import type { BulkStatusEditApplyResponse } from "../lib/aiTaskTypes";
...
function fail(error: string, status: number) {
  return data<BulkStatusEditApplyResponse>({ ok: false, error }, { status });
}
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!task || task.taskType !== "bulk_status_edit") {
  ...
  const rows = coerceBulkStatusEditRows(rawResult.rows);
  const writableRows = buildBulkStatusEditWritableRows(rows);
  ...
  try {
    ...
    return data<BulkStatusEditApplyResponse>({
      ok: true,
      succeeded: outcome.succeeded,
      failed: outcome.failed,
    });
  } catch (e) {
    console.error("[api.bulk-status-edit] apply failed:", e);
