/**
 * POST /api/bulk-seo-edit
 *
 * 批量 SEO 改写的写回入口，也是本功能唯一会向 Shopify 发 mutation 的地方。
 * 前置条件（全部满足才写）：
 *   1. 任务属于当前店铺、类型为 bulk_seo_edit、状态为 pending_review；
...
} from "../server/bulkSeoEdit/bulkSeoEditApply.server";
import { coerceBulkSeoEditRows } from "../lib/bulkSeoEdit";
import type { BulkSeoEditApplyResponse } from "../lib/aiTaskTypes";
...
function fail(error: string, status: number) {
  return data<BulkSeoEditApplyResponse>({ ok: false, error }, { status });
}
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!task || task.taskType !== "bulk_seo_edit") {
  ...
  const rows = coerceBulkSeoEditRows(rawResult.rows);
  const writableRows = buildBulkSeoEditWritableRows(rows);
  ...
  try {
    ...
    return data<BulkSeoEditApplyResponse>({
      ok: true,
      succeeded: outcome.succeeded,
      failed: outcome.failed,
    });
  } catch (e) {
    console.error("[api.bulk-seo-edit] apply failed:", e);
