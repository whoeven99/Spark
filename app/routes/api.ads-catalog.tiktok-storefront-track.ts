...
import { trackTiktokStorefrontTestEvent } from "../server/adsCatalog/tiktokPixelConfig.server";

/**
 * 店面公开端点：仅在 Ads Catalog 开启 Test Event Code 时，
 * 把浏览/加购等事件经 Events API 打进 TikTok Test Events（服务器侧）。
 * 无 admin session；靠「测试模式开关」+ CORS/限流约束。
 */
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  // 测试模式关闭时返回 200 + skipped，避免店面脚本报错刷屏
  return jsonResponse(
