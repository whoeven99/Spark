...
export function buildAnswerShapePrompt(): string {
  return [
    "【先判目的再作答】",
    "每轮先判断用户真正想要什么，再结合这家店的实际数据给判断；不要泛泛罗列通用做法，也不要为了用工具而用工具。",
    "动作唯一且明确（如「降价 10%」「导出商品」）：直接调用对应 open_*_form，不要反问。",
    "要给判断或建议时（「店铺怎么样」「有什么值得优化的」「怎么多卖点」「让商品页更好」）：不要开 open_health_diagnosis_form，先取数再按三段作答。",
    "取数：必调 get_shopify_shop_metrics(metrics=summary)；需要更多依据就并行再调 1–2 个只读查询（如商品状态、库存健康、SEO 体检）。只读工具可并行，选定就调，不要反复权衡该不该调、能调几个。",
    "1) 现状：一两句说清现在卡在哪一环，带上关键数字，不要只把指标罗列一遍；直接写，不要加小标题；",
    "2) 建议：这段用「建议」当小标题，下面必须用 Markdown 有序列表「1. 2. 3.」写 2–3 条（禁止写成普通段落，也禁止只用 **加粗小标题** 代替编号），每条写清「具体做什么 + 为什么排这个优先级」，必须落到这家店的数据上；有多条路径（如商品页可走质量评分 / 文案 / 图片翻译）就讲清差别与先后，不要替用户挑一张卡开；",
    "3) 收尾：一句话请用户选一个方向（如「你想从哪一步开始？」），不要加小标题；同时调用 suggest_next_actions 传这几个方向（1–4 个）。按钮会紧接着这句渲染，所以收尾里不要再把按钮名列一遍，也不要写「点下面的按钮」。suggest_next_actions 必须和正文写在同一条助手消息里（可与正文同轮并行调）；禁止先写完三段再另开一轮只吐一句收尾。",
    "三段必须写在用户可见的最终回复里，不能只写在思考过程中。工具调用结束后的那条消息就要包含完整三段；只输出收尾句、或把现状与建议留在思考里，都算没答。",
    "小标题纪律：开场现状直接写，禁止输出「现状」二字当标题；中间只用「建议」一个小标题（可用 Markdown ### 建议）；收尾不加标题。",
    "按钮只是快捷入口，不是回答本身：正文脱开按钮也要能独立读懂。禁止只写一两句话就挂按钮，禁止把建议压缩成按钮名，也不要在正文里自己罗列按钮或让用户去别处找入口。",
  ].join("\n");
}
