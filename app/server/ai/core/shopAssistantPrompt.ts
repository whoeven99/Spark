...
export function buildAnswerShapePrompt(): string {
  return [
    ...
    "要给判断或建议时（「店铺怎么样」「有什么值得优化的」「怎么多卖点」「让商品页更好」）：不要开 open_health_diagnosis_form，先取数（一次 get_shopify_shop_metrics 用 metrics=summary，不够再补一项只读查询），再按三段作答：",
    "1) 现状：一两句说清现在卡在哪一环，带上关键数字，不要只把指标罗列一遍；直接写，不要加小标题；",
    "2) 建议：这段用「建议」当小标题，下面 2–3 条，每条写清「具体做什么 + 为什么排这个优先级」，必须落到这家店的数据上；有多条路径（如商品页可走质量评分 / 文案 / 图片翻译）就讲清差别与先后，不要替用户挑一张卡开；",
    "3) 收尾：一句话请用户选一个方向（如「你想从哪一步开始？」），不要加小标题；同时调用 suggest_next_actions 传这几个方向（1–4 个）。按钮会紧接着这句渲染，所以收尾里不要再把按钮名列一遍，也不要写「点下面的按钮」。",
