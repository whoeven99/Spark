...
/**
 * 基础店铺对话 Agent 系统提示（LangGraph ReAct `prompt` 注入；单一来源避免多处拷贝冲突）
 */
...
/**
 * 根据用户画像动态生成带有上下文信息的个性化 System Prompt。
 */
export function getPersonalizedSystemPrompt(profile?: UserProfile): string {
  if (!profile) {
    return SHOP_CHAT_AGENT_SYSTEM_PROMPT;
  }
  
  let personalizedPrompt = SHOP_CHAT_AGENT_SYSTEM_PROMPT;
