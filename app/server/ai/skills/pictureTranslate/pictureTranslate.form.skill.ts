...
export const pictureTranslateFormToolDefinition: ToolDefinition = {
  ...
  systemPromptExtension:
    "当用户要翻译图片、商品图、截图中的文字时，必须调用 open_picture_translate_form 打开可编辑卡片，并从对话尽量预填 imageUrl、sourceLanguage、targetLanguage。调用后用与用户消息相同的语言说明用户可在卡片内完成配置并提交；不要固定使用中文。禁止在未成功调用 open_picture_translate_form 时声称「已打开卡片」或仅用文字描述表单；必须先发起工具调用。若用户已提供可访问的 HTTPS 图片 URL 与目标语言且要求立即翻译，可调用 picture_translate 直接执行。",
