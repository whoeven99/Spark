...
export const pictureTranslateFormToolDefinition: ToolDefinition = {
  ...
  systemPromptExtension:
    "当用户要翻译图片、商品图、截图中的文字，且需要在卡片里选择图片或确认语言时，必须调用 open_picture_translate_form，并从对话尽量预填 imageUrl、sourceLanguage、targetLanguage。调用后说明用户可在卡片内完成配置并提交。禁止在未成功调用 open_picture_translate_form 时声称「已打开卡片」。若用户已提供可访问的 HTTPS 图片 URL 与目标语言且要求立即翻译，可调用 picture_translate 直接执行。",
