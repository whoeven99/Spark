...
import { LIST_PRODUCT_SEO_TOOL_NAME } from "../bulkSeoEdit/listProductSeo.tool";
...
export const seoAuditSkillDefinition: ToolDefinition = {
  ...
  systemPromptExtension: [
    ...
    `   - 若没有 suggestedNextActions 字段：有 fixability=bulk_seo_edit 的 issue → 立刻调用 ${OPEN_BULK_SEO_EDIT_FORM_TOOL_NAME}，用该类 samples 的 productId/productTitle 预填；有 product_content → 立刻调用 ${OPEN_PRODUCT_IMPROVE_FORM_TOOL_NAME}（可先开一张代表性商品卡）；manual → 说明只能人工处理。`,
    "   - 商户点了 SEO 体检是为了改，不是听课；确认卡上他们还能改模板再提交。",
    "关于长度：阈值单位是「半角当量宽度」，一个汉字算 2。跟商户解释时说「约 30 个汉字」比说「60 字符」更好懂。",
    `商户如果已经知道要改什么、只想看现在写了什么，直接用 ${LIST_PRODUCT_SEO_TOOL_NAME} 就行，不必跑整店体检。`,
    "诚实边界：批量改 SEO 只支持 {title} / {vendor} / {productType}。补缺失描述很合适；标题重复若 vendor/productType 都一样，要如实说模板拆不开。",
