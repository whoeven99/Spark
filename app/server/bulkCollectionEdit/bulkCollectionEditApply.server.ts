...
import {
  fetchCollectionExclusionProductIds,
...
export function buildBulkCollectionEditUpdateInput(args: {
  ...
  if (args.action === "remove") {
    return {
      ...
      sourcesToUpdate: [{ condition: { id: args.sourceId, inclusion: { selectionsToRemove: selections } } }],
    };
  }
  const excluded = new Set(args.excludedProductIds ?? []);
  const clearExclusion = args.productIds
    ...
    sourcesToUpdate: [
      {
        condition: {
          ...
          ...(clearExclusion.length > 0 ? { exclusion: { selectionsToRemove: clearExclusion } } : {}),
        },
      },
    ],
  };
...
export async function applyBulkCollectionEdit(args: {
    ...
    if (collection) {
      console.warn(
        `${LOG_PREFIX} ${formatCollectionWritabilityDiagnosisLog({
          ...
          diagnosis: collection.diagnosis,
        })}`,
      );
    }
  ...
  const excludedProductIds =
    args.action === "add"
      ? Array.from(await fetchCollectionExclusionProductIds(args.admin, collection.sourceId))
      : [];
  ...
  console.info(
    `${LOG_PREFIX} shop=${args.shop} action=${args.action} succeeded=${succeeded} failed=${errors.length} pendingJob=${pendingJob}${
      errors[0] ? ` error=${JSON.stringify(errors[0].message)}` : ""
    }`,
  );
  return {
    at: new Date().toISOString(),
