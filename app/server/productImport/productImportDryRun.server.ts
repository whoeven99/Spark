...
import { formatCollectionWritabilityDiagnosisLog } from "../../lib/bulkCollectionEdit";
import { analyzeImportSheet, coerceProductImportOperations, collapseRepeatedImportIssues, filterActionableImportIssues } from "../../lib/productImport";
import { buildProductImportSheetPreview } from "../../lib/productImportSheetPreview";
import {
  buildProductImportPlan,
  countImportWritable,
  matchImportRecords,
  sampleImportPlanForReview,
  type ProductImportCollectionRef,
} from "../../lib/productImportPlan";
import { countWritableProductDeletes } from "../../lib/bulkProductDelete";
import { saveProductImportChangeset } from "./productImportChangeset.server";
...
async function runProductImportDryRun(params: EnqueueProductImportDryRunParams): Promise<void> {
  ...
  const actionableIssues = collapseRepeatedImportIssues(filterActionableImportIssues(plan.issues));
  const reviewPlan = sampleImportPlanForReview(plan);
  let changesetBlobPath: string | undefined;
  try {
    changesetBlobPath = await saveProductImportChangeset({
      shop: params.shop,
      taskId: params.taskId,
      plan,
    });
  } catch (error) {
    console.error(`${LOG_PREFIX} changeset blob failed taskId=${params.taskId}`, error);
    await failTask({
      taskId: params.taskId,
      errorMsg: buildAITaskMessage("productImport.dryRunFailed", t("productImport.dryRunFailed")),
      startedAt,
  ...
  const result: ProductImportTaskResult = {
    ...
    summary: {
      ...
      deletes: countWritableProductDeletes(plan.deleteRows),
      handles: plan.handleRows.filter((row) => !row.skipped).length,
    },
    priceRows: reviewPlan.priceRows,
    costRows: reviewPlan.costRows,
    tagRows: reviewPlan.tagRows,
    statusRows: reviewPlan.statusRows,
    fieldRows: reviewPlan.fieldRows,
    handleRows: reviewPlan.handleRows,
    collectionGroups: reviewPlan.collectionGroups,
    metafieldRows: reviewPlan.metafieldRows,
...
async function loadCollectionRefs(
  ...
  for (const option of listed) {
    ...
    if (snapshot && !snapshot.sourceId) {
      console.warn(
        `${LOG_PREFIX} ${formatCollectionWritabilityDiagnosisLog({
          collectionId: snapshot.id,
          collectionTitle: snapshot.title,
      ...
      if (log) {
        await appendLog({
          ...
          message: log.msg("productImport.logCollectionNotWritable", {
            collection: snapshot.title,
            reasonLabel: log.t(
              `productImport.collectionWritabilityReason.${snapshot.diagnosis.reason}`,
            ),
          }),
        });
      }
...
async function loadMembership(
  ...
  for (const collection of collections) {
    ...
    const { products } = await fetchCollectionMembershipByProductIds(
      ...
      collection.id,
      productIds,
      { maxProducts: Math.max(productIds.length, 1) },
    );
    map.set(
      collection.id,
