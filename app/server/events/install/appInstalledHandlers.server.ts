...
export async function handleAppInstalledEmail(
  ...
  try {
    ...
    try {
      console.info(`${LOG} before-profileSync shop=${event.shop}`);
      await syncProfile({
        shop: event.shop,
      ...
      console.info(
        `${LOG} after-profileSync shop=${event.shop} hasSnapshot=${Boolean(sessionSnapshot)}`,
      );
    } catch (error) {
      console.warn(
        `${LOG} profileSync failed shop=${event.shop}`,
        error,
      );
    }
