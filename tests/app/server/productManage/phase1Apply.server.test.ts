...
describe("product duplicate apply", () => {
  ...
  it("calls productDuplicate with draft + images", async () => {
    const { admin, calls } = createAdmin(() => ({
      data: {
        productDuplicate: {
          newProduct: { id: "gid://shopify/Product/99" },
          imageJob: { id: "gid://shopify/Job/1", done: false },
          userErrors: [],
        },
      },
    }));
    ...
    expect(calls[0]?.query).toContain("imageJob");
    expect(calls[0]?.query).toContain("productDuplicateOperation");
    expect(calls[0]?.query).not.toContain("productDuplicateJob");
    expect(calls[0]?.variables).toEqual({
      productId: "gid://shopify/Product/1",
      newTitle: "商品 (Copy)",
      includeImages: true,
  ...
  it("records GraphQL schema errors instead of treating the copy as created", async () => {
    const { admin } = createAdmin(() => ({
      errors: [{ message: "Field 'productDuplicateJob' doesn't exist on type 'ProductDuplicatePayload'" }],
    }));
    const outcome = await applyProductDuplicate({
      admin: asAdmin(admin),
      shop: "s",
    ...
    expect(outcome.errors[0]?.message).toContain("productDuplicateJob");
  });

  it("fails when Shopify returns an operation without a new product", async () => {
    const { admin } = createAdmin(() => ({
      data: {
        productDuplicate: {
          newProduct: null,
          productDuplicateOperation: { id: "gid://shopify/ProductDuplicateOperation/1", status: "ACTIVE" },
          userErrors: [],
        },
      },
    }));
    ...
    expect(outcome.succeeded).toBe(0);
    expect(outcome.errors[0]?.message).toContain("ACTIVE");
  });
});
...
describe("bulk collection edit apply", () => {
  ...
  it("adds with collectionUpdate source selections and flags pending jobs", async () => {
    ...
    expect(calls[2]?.query).toContain("collectionUpdate");
    ...
    expect(calls[4]?.query).toContain("collectionUpdate");
    expect(remove.pendingJob).toBe(true);
  });

  it("clears manual exclusion when adding a previously excluded product", async () => {
    const { admin, calls } = createAdmin((call) => {
      if (String(call.query).includes("collectionUpdate")) {
        return { data: { collectionUpdate: { job: null, userErrors: [] } } };
      }
      if (String(call.query).includes("BulkCollectionEditExclusions")) {
        return {
          data: {
            node: {
              exclusion: {
                selections: {
                  ...
                  nodes: [{ product: { id: "gid://shopify/Product/1" } }],
                },
              },
            },
          },
    ...
    await applyBulkCollectionEdit({
      admin: asAdmin(admin),
      shop: "s",
      collectionId: "gid://shopify/Collection/1",
      action: "add",
      rows: [collectionRow(), collectionRow({ productId: "gid://shopify/Product/2" })],
    });
    expect(calls[2]?.variables).toEqual({
      collection: {
        id: "gid://shopify/Collection/1",
